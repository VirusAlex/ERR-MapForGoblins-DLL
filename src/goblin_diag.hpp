...
// Lightweight injection-status registry. Each inject site reports its outcome
// here; the overlay Debug tab renders a human-readable report a player can
// screenshot for a bug report (so we can see WHAT failed and WHY without a log).
// All setters are thread-safe: injects run on the game's loader thread, the
...
namespace goblin::diag
{
    ...
    // Marker iconId remap (markers pointed at our injected frames).
